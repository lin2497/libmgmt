...
public partial class personal_information : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (current_user != null)
            {
                ...
                UserInfo info = (new CData()).getUserInfoByUserID(current_user.Id);
                if (info != null)
                {
    ...
    protected void btn_renew_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            UserInfo info = (new CData()).getUserInfo(Int32.Parse(Session["editInfoID"].ToString()));
            ...
            (new CData()).saveUserInfo(info);
            Response.Redirect("/message.aspx?msg=" + "User info updated!");
